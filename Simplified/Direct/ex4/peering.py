...
Rayleigh_wave = forward(this_function, 0.93, depth, shear_wave, list_wavelength)


def conversion_parameter(list_wavelength, Rayleigh_wave, alpha, coeff):

    cdepth = alpha * list_wavelength
    cshear_wave = (coeff**-1) * Rayleigh_wave
...
def iter_inversion(which_function, list_wavelength, Rayleigh_wave, alpha, coeff, num_ilayer):
    ...
    if case == 'top_down':
        ...
        if (num_ilayer > len(list_wavelength)):
            ...
            print ('Error: Number of inverted layers is more than the length of wavelength')
        ...
        if (num_ilayer == 1):
            ...
            idepth = [cdepth[num_ilayer-2], np.inf]
    ...
    W = np.dot(weight_mat.T, weight_mat)
    ATb = np.dot(weight_mat.T, ishear_wave_data)
    ishear_wave = np.dot(np.linalg.inv(W), ATb)
...
def plot_figure():
    ...
    fig, ax = plt.subplots()
    ishear_wave = iter_inversion(this_function, list_wavelength, Rayleigh_wave, alpha, 0.93, at_iter)
    print(ishear_wave)
    # ax.step(ishear_wave, alpha * list_wavelength[:at_iter])
    # ax.invert_yaxis()
    # plt.show()
