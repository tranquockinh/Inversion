...
class dispersion_analysis(object):
    ...
    def dispersion_image(self):
        self.MASW_Despersion_imaging()
    ...
    def extract_dispersion_curve(self):
        ...
        Amax_fvec = Amax_fvec[ii] # filter of frequencies
...
delta_c_test = 0.5
num_layers = 6
shear_wave = np.array([1400,1400,1400,1400,1400,1400,1400])
thickness = np.array([1,1,2,2,4,5,np.inf])
p_wave = np.array([75,90,150,180,240,290,290])
unit_weight = [1850,1850,1850,1850,1850,1850,1850]
c_test_array = np.arange(c_test_min,c_test_max+delta_c_test,delta_c_test)
class seismic_backward(object):
    ...
    def stiffness_matrix(self,c_test,k,h,alpha,beta,rho,n):
        ...
        DOFs = np.arange(2*(n-1),2*(n-1)+3+1,1)
        for row in DOFs:
            for col in DOFs:
                row_idx = np.array(np.where(DOFs==row)).flatten()[0]
                col_idx = np.array(np.where(DOFs==col)).flatten()[0]
