...
def Monte_Carlo_Analysis(c_min,c_max,c_step,delta_c,n,n_unsat,alpha_initial,\
    ...
    for w in range(N_max):
        ...
        ax1.set_xlabel('Number of iteration',fontsize=11)
        ax1.set_ylabel('Misfit (%)',fontsize=11)
        if e_test > e_opt:
    ...
    ax1.plot(all_iter,all_misfit,'-r',label='Sampling',linewidth=1)
    ax1.plot(iter,misfit,'-ob',label='Accepted')
    ax1.legend()
    ax1.set_xlabel('Number of iteration - Min.misfit={}%'.format(min(misfit)),fontsize=11)
    ax1.set_ylabel('Misfit (%)',fontsize=11)
    ax1.axhline(y= min(misfit),xmin=0,xmax=N_max,linewidth=1,color ='k')
    return store_all
...
def visualization(c_OBS,lambda_OBS,n,store_all):
    ...
    ax2.set_xlabel('Phase velocity (m/s)',fontsize=11)
    ax2.set_ylabel('Wavelength (m)',fontsize=11)
    plt.rc('xtick', labelsize=11) 
    plt.rc('ytick', labelsize=11)
    ax2.invert_yaxis()

    ax3.set_xlim(100,220)
    ax3.set_xlabel('Phase velocity (m/s)',fontsize=11)
    ...
    for j in range(NoPlot_all):
        ...
        ax2.plot(c_plot,lambda_plot,linewidth = 1*j/N_max)
        ax3.plot(plot_beta,plot_layer_depth,linewidth = 1*j/N_max)
        plt.draw()
        plt.pause(0.01)

    ax2.plot(c_OBS,lambda_OBS,'-ok',linewidth=2,label='OBS D-curve')
    ax3.plot(plot_beta,plot_layer_depth,'-or',linewidth=2,label = 'Best fit profile')
    ax2.legend()
    ax3.legend()
    plt.show(block=True)
