...
MaxDepth = 16.0
h_initial = [2,2,4,6]
beta_initial = [1.09*c_OBS[0], 1.09*c_OBS[9], 1.09*c_OBS[16], \
    1.09*c_OBS[22], 1.09*c_OBS[-1]]
n_unsat = 1
nu_unsat = 0.3
...
def Fast_delta_matrix(c,k,n,alpha,beta,rho,h):
    # initialization
    X_temp = np.array([2*(2-c**2/beta[0]**2), \
                        -(2-c**2/beta[0]**2)**2, 0, 0, -4])
    X = [(rho[0]*beta[0]**2)**2 * X_temp[i] for i in range(len(X_temp))]
    # layer recursion
    for i in range(n):
        X = Fast_delta_recersion(c,k,alpha[i],beta[i],beta[i+1],rho[i],\
                                                    rho[i+1],h[i],X)
...
def Theorectical_dispersion_curve(c_min,c_max,c_step,Lambda,n,\
                                    alpha,beta,rho,h,delta_c):
    ...
    for j in range(len(k)):
            for m in range(m_loc-1,len(c_test)):
                ...
                if m == (m_loc-1):
                    sign_old = np.sign(Fast_delta_matrix(c_test[0],k[j],n,\
                        alpha,beta,rho,h))
                else:
                    sign_old = signD
                signD = np.sign(D[j,m])
...
def visualization(c_OBS,lambda_OBS,n,store_all):
    ...
    return NoPlot_all,order_all_plot,c_plot,lambda_plot,plot_layer_depth,plot_beta


def Monte_Carlo_Analysis(c_min,c_max,c_step,delta_c,n,n_unsat,alpha_initial,\
    nu_unsat,beta_initial,
                rho,h_initial,N_reversals,c_OBS,lambda_OBS,up_low_boundary,\
                    c_OBS_up,c_OBS_low,
                b_S,b_h,N_max,e_max):
    
    c_t, lambda_t = Theorectical_dispersion_curve(c_min,c_max,c_step,\
        lambda_OBS,n,alpha_initial,
    ...
    for w in range(N_max):
        ...
        # NoPlot_all,order_all_plot,c_plot,lambda_plot,plot_layer_depth,plot_beta \
        #                             = visualization(c_OBS,lambda_OBS,n,store_all)
        
        # ax2.set_xlim(100,200)
        # ax2.set_xlabel('Phase velocity (m/s)',fontsize=12)
        # ax2.set_ylabel('Wavelength (m)',fontsize=12)
        # plt.rc('xtick', labelsize=11) 
        # plt.rc('ytick', labelsize=11)
        # ax2.invert_yaxis()

        # ax3.set_xlim(100,220)
        # ax3.set_xlabel('Phase velocity (m/s)',fontsize=12)
        # ax3.set_ylabel('Wavelength (m)',fontsize=12)
        # plt.rc('xtick', labelsize=11) 
        # plt.rc('ytick', labelsize=11)
        # ax3.invert_yaxis()
        ...
        #     ax2.plot(c_plot,lambda_plot,linewidth = j/N_max)
        #     ax2.plot(c_OBS,lambda_OBS,'-ok',linewidth=2)
        #     ax2.legend(['OBS D-curve'])
        #     ax3.plot(plot_beta,plot_layer_depth,linewidth = j/N_max)
        #     ax3.legend(['Best fit profile'])
        #     plt.draw()
        #     plt.pause(0.1)
        
        # ax3.plot(plot_beta,plot_layer_depth,'-or',linewidth=2)
        # plt.show(block=True)
        
        all_iter.append(w)
        all_misfit.append(e_test*100)
        
        ax1.plot(all_iter,all_misfit,'-or')
        ax1.set_xlabel('Number of iteration',fontsize=12)
        ax1.set_ylabel('Misfit (%)',fontsize=12)
        if e_test > e_opt:
    ...
    ax1.plot(all_iter,all_misfit,'-or',label='Sampling')
    ax1.plot(iter,misfit,'-ob',label='Accepted')
    ax1.legend()
    ax1.set_xlabel('Number of iteration',fontsize=12)
    ax1.set_ylabel('Misfit (%)',fontsize=12)
    plt.gcf().show() 
